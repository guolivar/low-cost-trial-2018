...
# Choose ODINS
x_dev <- c(0,0)
x_dev[1] <- grep(x=curr_data$ODIN,pattern = '0052')
x_dev[2] <- grep(x=curr_data$ODIN,pattern = '0065')
...
print(x_now)
# UTC time start
t_start <- as.numeric(as.POSIXct("2018/10/28 12:00:00",tz = "GMT-12"))
# UTC time end ... now
...
timePlot(subset(all_data,serialn == curr_data$ODIN[x_dev[1]]),
...
timePlot(subset(all_data,serialn == curr_data$ODIN[x_dev[2]]),
         pollutant = c('PM1','PM2.5','PM10'),
         main = curr_data$ODIN[i_dev],
         avg.time = avg_plot,
         group = TRUE,
...
timeVariation(wide_data,pollutant = c('PM2.5.x','PM2.5.y'))

timePlot(wide_data,pollutant = c('PM2.5.x','PM2.5.y'),
         avg.time = avg_plot,
         group = TRUE,
         ylim = c(0,50))

timePlot(subset(wide_data,date > as.POSIXct("2018-08-27 21:00:00",tz = 'GMT')),
         pollutant = c('Temperature.x','Temperature.y'),
         name.pol = c('Indoor','Outdoor'),
         main = 'Indoor and Outdoor comparison',
         key.columns = 1,
         avg.time = '5 min',
         group = TRUE,
         ylim = c(0,50))
